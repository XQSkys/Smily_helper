...
public class Smily_Helper_V_1_8_6 extends PApplet {
...
outputbw = createWriter("work/list.txt"); ch=1;}
/*
if(ch1==0){
  outputg = createWriter("w"+ordner+"/gSavesMovie/gMovie000000"+iv+".txt"); 
outputb = createWriter("w"+ordner+"/bSavesMovie/bMovie000000"+iv+".txt");  
outputr = createWriter("w"+ordner+"/rSavesMovie/rMovie000000"+iv+".txt");  
outputbw = createWriter("w"+ordner+"/bwSavesMovie/bwMovie000000"+iv+".txt"); 
ch=1;}
*/
  noStroke();
  if (scb<=254) {
 ...
 //     println("y"+sumly);
  ...
  if ((mouseoverx>=0&&mouseoverx<=70)&&(mouseovery>=0&&mouseovery<=30)) {   
    ...
    if   (mousePressed && (mouseButton == LEFT)&&num>=2) {
...
li=0;
  ...
  String fff="eof";
      ...
      String[] linesr = loadStrings("work/listr.txt");
String[] linesg = loadStrings("work/listg.txt");
String[] linesb = loadStrings("work/listb.txt");
String[] linesbw = loadStrings("work/list.txt");


println("there are " + linesbw.length + " lines");
lx=1;
ly=0;
li=0;
while (gone!=1){
 
  if ((linesbw.length-1)==li) {gone=1;}
  
  else  {
  
if (ggg.equals(linesbw[li]) == true ) {
 ...
 bytr = PApplet.parseInt(linesr[li]);
 bytg = PApplet.parseInt(linesg[li]);
 bytb = PApplet.parseInt(linesb[li]);
 bytbw= PApplet.parseInt (linesbw[li]);
...
//
    ...
    if (mousePressed && (mouseButton == RIGHT)&&num>=2) {
...
//if (bild == null) { iv--; if (iv<0)iv=0;}
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Smily_Helper_V_1_8_6" };
    if (passedArgs != null) {
